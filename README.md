...
```
aws s3api put-bucket-cors --bucket <bucket-name> --cors-configuration file://cors.json --endpoint-url https://s3.fr-par.scw.cloud --profile scaleway
```

Verify current CORS config on Scaleway S3 bucket
```
aws s3api get-bucket-cors --bucket <bucket-name> --endpoint-url https://s3.fr-par.scw.cloud --profile scaleway
```